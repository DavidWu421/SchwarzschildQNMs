...
@testset "ScalarKerrPertToSchwMany" begin 
    ...
    CSV.write("/home/dgw763/Documents/LinearizedSpin/ScalarPerturbations/KerrPertToSchw/freqpertsmatrix.csv", freqpertsmatrix)
end
...
@testset "ScalarLinearizedJPMany" begin   #CHECKED AND GOOD
    ...
    for l in 0:lmax
        ...
        for n in 0:2
            for a in [0.0]
                ...
                TheContourup = C1up⊕C2up⊕C3up
                # Make the operators
                ∂ϵO0 = OperatorSandwich(ψ,∂ϵO,weight,ψ).Op
                ∂ωO0 = OperatorSandwich(ψ,∂ωO,weight,ψ).Op
                println("Made Operators")
                ∂ϵ𝒪0 = Integrate(∂ϵO0, TheContourup,reltol=1e-4)[1]
                ∂ω𝒪0 = Integrate(∂ωO0, TheContourup,reltol=1e-4)[1]
                @show ∂ϵ𝒪0
                @show ∂ω𝒪0
                @show δω = -(∂ϵ𝒪0/∂ω𝒪0)
                push!(freqpertsmatrix, (l, m, n, a, δω))
    ...
    CSV.write("/home/dgw763/Documents/LinearizedSpin/Final2ShiftsAndChecks/ScalarEikonalJPFreqs.csv", freqpertsmatrix)
end
...
@testset "KerrNewmanpertm2Fitting" begin
    ...
    for elem in modelist
        ...
        freqpertsmatrix =DataFrame(l = Int[], m = Int[], n = Int[], a = Float64[] , δω = ComplexF64[])
        filenamestart="/home/dgw763/Documents/KerrNewman/Fitting/freqpertsmatrix440retest"
        filenameend= ".csv"
        lstr=string(l)
        mstr=string(m)
        nstr=string(n)
...
@testset "Eikonal" begin
    ...
    for l in 2:lmax
        ...
        for n in [3]
            for a in [0.]
                ...
                # pert_ϵ=1
                # (∂ω𝒪plusSchw,∂ω𝒪plusSchwerr)= Integrate(∂ωOplusSchw, TheContourup,pertparam=pert_ϵ,abstol=1e-6)
                # ∂ω𝒪minusSchw= conj(Integrate(∂ωOminusSchw, TheContourdown,pertparam=pert_ϵ,abstol=1e-6)[1])
                # @show ∂ω𝒪plusSchw
                # @show ∂ω𝒪minusSchw
                # (ℋlinplusSchw, ℋlinplusSchwerr)= Integrate(HlinplusSchw, TheContourup,pertparam=pert_ϵ,abstol=1e-6)
                # ℋlinminusSchw= conj(Integrate(HlinminusSchw, TheContourdown,pertparam=pert_ϵ,abstol=1e-6)[1])
                # @show ℋlinplusSchw
                # @show ℋlinminusSchw    
                # (ℐlinplusSchw, ℐlinplusSchwerr)= Integrate(IlinplusSchw, TheContourup,pertparam=pert_ϵ,abstol=1e-6)
                ...
                # pert_ϵ=1
                # (∂ω𝒪plusSchw,∂ω𝒪plusSchwerr)= Integrate(∂ωOplusSchw, TheContourup,pertparam=pert_ϵ,reltol=1e-1, abstol=1e-2)
                # ∂ω𝒪minusSchw= conj(Integrate(∂ωOminusSchw, TheContourdown,pertparam=pert_ϵ,reltol=1e-1, abstol=1e-2)[1])
                # @show ∂ω𝒪plusSchw
                # @show ∂ω𝒪plusSchwerr
                # @show ∂ω𝒪minusSchw
                # (ℋlinplusSchw, ℋlinplusSchwerr)= Integrate(HlinplusSchw, TheContourup,pertparam=pert_ϵ,reltol=1e-1, abstol=1e-2)
                # ℋlinminusSchw= conj(Integrate(HlinminusSchw, TheContourdown,pertparam=pert_ϵ,reltol=1e-1, abstol=1e-2)[1])
                # @show ℋlinplusSchw
                # @show ℋlinplusSchwerr
                # @show ℋlinminusSchw    
                # (ℐlinplusSchw, ℐlinplusSchwerr)= Integrate(IlinplusSchw, TheContourup,pertparam=pert_ϵ,reltol=1e-1, abstol=1e-2)
                # ℐlinminusSchw= conj(Integrate(IlinminusSchw, TheContourdown,pertparam=pert_ϵ,reltol=1e-1, abstol=1e-2)[1])
                # @show ℐlinplusSchw
                # @show ℐlinplusSchwerr
                # @show ℐlinminusSchw
                # ω2s=Computeω2(∂ω𝒪plusSchw,∂ω𝒪minusSchw,ℋlinplusSchw,ℋlinminusSchw,ℐlinplusSchw,ℐlinminusSchw,ψ)
                # @show ω2s

                pert_ϵ=1
                (∂ω𝒪plusSchw,∂ω𝒪plusSchwerr)= Integrate(∂ωOplusSchw, TheContourup,pertparam=pert_ϵ,reltol=1e-3, abstol=1e-6)
                ∂ω𝒪minusSchw= conj(Integrate(∂ωOminusSchw, TheContourdown,pertparam=pert_ϵ,reltol=1e-3, abstol=1e-6)[1])
                @show ∂ω𝒪plusSchw
                @show ∂ω𝒪plusSchwerr
                @show ∂ω𝒪minusSchw
                (ℋlinplusSchw, ℋlinplusSchwerr)= Integrate(HlinplusSchw, TheContourup,pertparam=pert_ϵ,reltol=1e-3, abstol=1e-6)
                ℋlinminusSchw= conj(Integrate(HlinminusSchw, TheContourdown,pertparam=pert_ϵ,reltol=1e-3, abstol=1e-6)[1])
                @show ℋlinplusSchw
                @show ℋlinplusSchwerr
                @show ℋlinminusSchw    
                (ℐlinplusSchw, ℐlinplusSchwerr)= Integrate(IlinplusSchw, TheContourup,pertparam=pert_ϵ,reltol=1e-3, abstol=1e-6)
                ℐlinminusSchw= conj(Integrate(IlinminusSchw, TheContourdown,pertparam=pert_ϵ,reltol=1e-3, abstol=1e-6)[1])
                @show ℐlinplusSchw
                @show ℐlinplusSchwerr
                @show ℐlinminusSchw
                ω2s=Computeω2(∂ω𝒪plusSchw,∂ω𝒪minusSchw,ℋlinplusSchw,ℋlinminusSchw,ℐlinplusSchw,ℐlinminusSchw,ψ)
    ...
    CSV.write("/home/dgw763/Documents/LinearizedSpin/Final3ShiftsAndChecks/EikonalJPFreqs.csv", freqpertsmatrix)
end
...
@testset "LinearizedJPMany" begin
        ...
        for m in 2:min(l,3)
            for n in 0:2
                for a in [0.0, 0.01, 0.02]
                    ...
                    ∂ω𝒪plusSchw= Integrate(∂ωOplusSchw, TheContourup,pertparam=pert_ϵ,abstol=1e-6,reltol=1e-2)[1]
                    ∂ω𝒪minusSchw= conj(Integrate(∂ωOminusSchw, TheContourdown,pertparam=pert_ϵ,abstol=1e-6,reltol=1e-2)[1])
                    @show ∂ω𝒪plusSchw
                    @show ∂ω𝒪minusSchw
                    ℋlinplusSchw= Integrate(HlinplusSchw, TheContourup,pertparam=pert_ϵ,abstol=1e-6,reltol=1e-2)[1]
                    ℋlinminusSchw= conj(Integrate(HlinminusSchw, TheContourdown,pertparam=pert_ϵ,abstol=1e-6,reltol=1e-2)[1])
                    ℐlinplusSchw= Integrate(IlinplusSchw, TheContourup,pertparam=pert_ϵ,abstol=1e-6,reltol=1e-2)[1]
                    ℐlinminusSchw= conj(Integrate(IlinminusSchw, TheContourdown,pertparam=pert_ϵ,abstol=1e-6,reltol=1e-2)[1])
                    ω2s=Computeω2(∂ω𝒪plusSchw,∂ω𝒪minusSchw,ℋlinplusSchw,ℋlinminusSchw,ℐlinplusSchw,ℐlinminusSchw,ψ)
                    @show ω2s
    ...
    CSV.write("/home/dgw763/Documents/LinearizedSpin/Final2ShiftsAndChecks/JPShifts.csv", freqpertsmatrix)
end

@testset "LinearizedJP" begin     #CHECKED AND GOOD
    pert_ϵ=1
    
    ψ = qnmfunctionnew(-2,2,2,1,0.)
    ψconj = qnmfunctionnew(-2,2,2,1,0.,is_conjugate=true)

    ψm = qnmfunctionnew(-2,2,2,1,0.,is_minus=true)
    ψmconj = qnmfunctionnew(-2,2,2,1,0.,is_minus=true,is_conjugate=true)
    ...
    ∂ω𝒪plus= Integrate(∂ωOplus, TheContourup,pertparam=pert_ϵ,abstol=1e-6, reltol=1e-2)[1]
    ∂ω𝒪minus= conj(Integrate(∂ωOminus, TheContourdown,pertparam=pert_ϵ,abstol=1e-6, reltol=1e-2)[1])
    @show ∂ω𝒪plus
    @show ∂ω𝒪minus
    ℋlinplus= Integrate(Hlinplus, TheContourup,pertparam=pert_ϵ,abstol=1e-6, reltol=1e-2)[1]
    ℋlinminus= conj(Integrate(Hlinminus, TheContourdown,pertparam=pert_ϵ,abstol=1e-6, reltol=1e-2)[1])
    @show ℋlinplus
    @show ℋlinminus
    ℐlinplus= Integrate(Ilinplus, TheContourup,pertparam=pert_ϵ,abstol=1e-6, reltol=1e-2)[1]
    ℐlinminus= conj(Integrate(Ilinminus, TheContourdown,pertparam=pert_ϵ,abstol=1e-6, reltol=1e-2)[1])
    @show ℐlinplus
    @show ℐlinminus
